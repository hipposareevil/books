      ...
      <div class="level-left">
        <p class="level-item">
          <span class="is-size-3"
                style="padding-right: 5px;">Search</span>
      ...
      <div class="level-right">
        <div class="level-item has-text-centered">
          <div class="level-item">
              ...
              <RightHeader :numberOfThings="getCurrentLength"
...
<script>
  ...
  export default {
    ...
    components: { BooksAsList, BooksAsGrid, ViewHeader, RightHeader, InfiniteLoading },
    // Data for this component
    watch: {
      books: function (val, oldVal) {
        this.BooksJson = val
      },
      '$store.getters.search': function (val, oldVal) {
     ...
     */
    computed: {
      // Return length of the current set of books
      getCurrentLength: function () {
        return this.AllData.BooksJson.length
      },
      // Show search stuff if we have over 3 chars
      showSearch: function () {
        if (this.searchBookString.length > 3) {
          return true
        } else {
          return false
        }
    ...
    mounted: function () {
      ...
      // Get string
      this.updateSearchString(this.$store.getters.search)
    ...
    methods: {
      ...
      infiniteHandler ($state) {
        ...
        // If we are not showing the search
        // tell the infinite scroller to not scroll
        ...
        if (self.searchBookString.length > 3) {
          ...
          if (self.searchBookString !== '') {
            params.title = self.searchBookString
            params.author_name = self.searchBookString
          }
          ...
          if (this.AllData.getAll) {
            ...
            this.$axios.get(url, {
              ...
              .then((response) => {
                ...
                if (self.AllData.BooksJson.length === numBooks) {
